...
def read(casePath, debug=False):
    ...
    if not os.path.exists(casePath):
        raise ValueError("Provided path to .vtp file invalid!")
...
def average_patch_data(patchBlocks, boundaryData, nSamples, bounds, algorithm, debug):
    ...
    for boundary in boundaryData:
        ...
        for seed in tqdm(range(nSeedPoints)):
#            if debug:
...
#                    print(avergFields[field].shape, data[field].shape)
...
def create_boundary_polydata(patchBlocks, patchData, bounds, debug):
    ...
    for patchName in get_block_names(patchBlocks):
        ...
        # Hardcoding bounds..
        b4Points = np.ones(boundaryIPoints.shape[0])*(0.0)
        b5Points = np.ones(boundaryIPoints.shape[0])*(-0.0493)
...
def main():
    ...
    try:
        ...
        pass
    if dry:
        print("This is a dry run, no averaging will be actually performed")
    ...
    internalBlock = reader.GetOutput()
    bounds = internalBlock.GetBounds()
    if debug:
        print("The z bounds are " + str(bounds[4]), " ", str(bounds[5]))


    boundaryReader = vtk.vtkXMLMultiBlockDataReader()
    boundaryReader.SetFileName(config["patchData"])
    boundaryReader.Update()
    patchBlocks = boundaryReader.GetOutput().GetBlock(0)

    # The polyData for the 2d fields, copied from the seed patch
    internalData = vtk.vtkPolyData()
    internalData.ShallowCopy(internalBlock)
    internalData.BuildLinks()

    internalCellData = internalData.GetCellData()
    nFields = internalCellData.GetNumberOfArrays()
    print("Number of arrays", nFields)
    internalDataWrap = dsa.WrapDataObject(internalData)

        # Copy fields with columnAverage() into original
        #for field in range(nFields):
        #    name = internalCellData.GetArrayName(field)
        #    if "column" in name:
        #        strippedName = name[16:-1]


#                internalDataWrap.CellData[strippedName] = internalDataWrap.CellData[name]
#                internalDataWrap.RemoveArray(name)
    ...
    print(patchBlocks.GetNumberOfBlocks())
